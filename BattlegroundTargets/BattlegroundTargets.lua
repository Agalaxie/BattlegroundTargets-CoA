...
-- Conquest of Azeroth can expose custom class tokens that do not exist in the
-- standard 3.3.5 class tables.  Keep the target button usable in that case;
-- the class icon is simply omitted and the frame uses this neutral colour.
local unknownClassColor = { r = 0.45, g = 0.45, b = 0.45 };

-- CoA ships a native 8 x 4 atlas of circular class emblems.  The textures
-- remain in the player’s installed client; this addon only references them.
local coaClassIconTexture = "Interface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes-Round";
...
local function CoAClassIcon(column, row)
	return {
		column / 8, (column + 1) / 8,
		row / 4, (row + 1) / 4,
		texture = coaClassIconTexture
	};
end

-- Coordinate map for Ascension's native circular class-emblem atlas.
classes.BARBARIAN     = CoAClassIcon(1, 1);
classes.CHRONOMANCER  = CoAClassIcon(1, 0);
classes.CULTIST       = CoAClassIcon(2, 2);
classes.DEMONHUNTER   = CoAClassIcon(4, 0);
classes.FLESHWARDEN   = CoAClassIcon(6, 1);
classes.GUARDIAN      = CoAClassIcon(7, 0);
classes.MONK          = CoAClassIcon(3, 1);
classes.NECROMANCER   = CoAClassIcon(4, 1);
classes.PROPHET       = CoAClassIcon(7, 1);
classes.PYROMANCER    = CoAClassIcon(0, 2);
classes.RANGER        = CoAClassIcon(1, 2);
classes.REAPER        = CoAClassIcon(1, 2);
classes.SONOFARUGAL   = CoAClassIcon(5, 2);
classes.SPIRITMAGE    = CoAClassIcon(6, 2);
classes.STARCALLER    = CoAClassIcon(7, 2);
classes.STORMBRINGER  = CoAClassIcon(0, 3);
classes.SUNCLERIC     = CoAClassIcon(0, 1);
classes.TINKER        = CoAClassIcon(2, 3);
classes.WILDWALKER    = CoAClassIcon(5, 3);
classes.WITCHDOCTOR   = CoAClassIcon(6, 3);
classes.WITCHHUNTER   = CoAClassIcon(4, 3);
...
local sortfunc13 = function(a, b) -- Класс/Имя | 13
	local classA = class_BlizzSort[ a.classToken ] or 999;
	local classB = class_BlizzSort[ b.classToken ] or 999;
...
local sortfunc11 = function(a, b) -- Класс/Имя | 11
	local classA = class_LocaSort[ a.classToken ] or 999;
	local classB = class_LocaSort[ b.classToken ] or 999;
...
function BattlegroundTargets:MainDataUpdate()
	...
	for i = 1, currentSize do
		if ENEMY_Data[i] then
			...
			local classColor = classcolors[qclassToken] or unknownClassColor;
			local classIcon = classes[qclassToken];
			local colR = classColor.r;
			local colG = classColor.g;
			local colB = classColor.b;
			...
			if(ButtonClassIcon and classIcon) then
				GVAR_TargetButton.ClassTexture:SetTexture(classIcon.texture or "Interface\\WorldStateFrame\\Icons-Classes");
				GVAR_TargetButton.ClassTexture:SetTexCoord(classIcon[1], classIcon[2], classIcon[3], classIcon[4]);
				GVAR_TargetButton.ClassTexture:Show();
			else
				GVAR_TargetButton.ClassTexture:Hide();
			end
		...
		if(OPT.ButtonRangeCheck[currentSize]) then
			...
			elseif(OPT.ButtonTypeRangeCheck[currentSize] >= 2) then
				if(ranges[playerClassEN]) then
					...
					-- CoA classes do not have a matching WotLK spell in the range
					-- table.  Combat-log range information is still safe and useful,
					-- so fall back to it instead of reporting an error on every BG.
					Print("INFO", "Combat-log range fallback enabled for", playerClassEN..".");
					BattlegroundTargets:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
				end
			end
		end
